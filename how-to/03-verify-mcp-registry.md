...
The `GroupDocs.Markdown.Mcp` package is an **MCP server** (the csproj declares
...
[nuget.org/packages/GroupDocs.Markdown.Mcp](https://www.nuget.org/packages/GroupDocs.Markdown.Mcp)
...
- `command: "dnx"`
- `args: ["GroupDocs.Markdown.Mcp@26.5.0", "--yes"]`
...
```bash
curl -L -o pkg.nupkg \
  https://www.nuget.org/api/v2/package/GroupDocs.Markdown.Mcp/26.5.0
...
```json
{
  ...
  "name": "io.github.groupdocs-markdown/groupdocs-markdown-mcp",
  "version": "26.5.0",
  "packages": [ { "registryType": "nuget", "identifier": "GroupDocs.Markdown.Mcp", ... } ],
  "repository": { "url": "https://github.com/groupdocs-markdown/GroupDocs.Markdown.Mcp" }
}
```
...
3. Search for `groupdocs-markdown` or `markdown`.
...
- `dnx` successfully resolves `GroupDocs.Markdown.Mcp@26.5.0` from nuget.org.
...
curl -s https://registry.modelcontextprotocol.io/v0/servers \
  | jq '.[] | select(.name | contains("groupdocs-markdown"))'
```
...
- `ToolDiscoveryTests.ServerInfo_AdvertisesGroupDocsMarkdownMcp` — the server
  reports `serverInfo.name == "GroupDocs.Markdown.Mcp"` and a non-empty version
  that matches the package under test.
- `ToolDiscoveryTests.ListTools_ExposesConvertAndGetDocumentInfo` — both MCP tools (`convert_to_markdown`, `get_document_info`) are advertised.
