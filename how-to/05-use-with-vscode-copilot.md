...
2. Open Copilot chat.
3. Click the tools / MCP icon — `groupdocs-markdown` should appear with
...
The [NuGet.org package page](https://www.nuget.org/packages/GroupDocs.Markdown.Mcp)
...
```
@workspace convert /docs/contract.pdf to Markdown

Use convert_to_markdown to export business-plan.docx as Markdown, text only.

How many pages does report.pdf have? Use get_document_info.
```
...
Without a license the server still works — converted Markdown may be limited and
carries an evaluation notice (see
