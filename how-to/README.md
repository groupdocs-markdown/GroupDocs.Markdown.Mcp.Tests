...
[`GroupDocs.Markdown.Mcp`](https://www.nuget.org/packages/GroupDocs.Markdown.Mcp).
...
| 07 | [Use with Cursor](07-use-with-cursor.md) | Connect from Cursor's Agent (`mcpServers` key; Windows `dnx` SSL workaround). |
...
- All guides target `GroupDocs.Markdown.Mcp@26.7.0`. Substitute a newer version
  freely — the interfaces haven't changed.
- Tools exposed on the wire are `convert_to_markdown` and `get_document_info`
  (snake_case).
- The server runs fine **without a license**: both tools work in evaluation
  mode — converted Markdown may be limited and carries an evaluation notice.
  A `GroupDocs.Total.lic` removes those limits. See each guide's "License" section.
- The server does **not** expose a `compose_from_markdown` tool — reverse
  ...
  GroupDocs.Markdown 26.3.0 (the engine version). Use the GroupDocs.Conversion
  MCP for Markdown → DOCX / PDF / HTML in the interim.
