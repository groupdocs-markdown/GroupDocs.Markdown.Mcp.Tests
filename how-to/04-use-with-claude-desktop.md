...
Claude to convert your documents (PDF, DOCX, XLSX, EPUB, …) to clean Markdown
and inspect document info.
...
```json
{
  "mcpServers": {
    "groupdocs-markdown": {
      "type": "stdio",
      "command": "groupdocs-markdown-mcp",
      "env": {
        "GROUPDOCS_MCP_STORAGE_PATH": "/Users/you/Documents"
      }
    }
...
Requires you've already run `dotnet tool install -g GroupDocs.Markdown.Mcp`
...
2. Click the **🔨 tools** icon in the composer — you should see
   `convert_to_markdown` and `get_document_info`
   listed under `groupdocs-markdown`.
...
Claude will call `convert_to_markdown` and `get_document_info` and compose its
answer from the tool results.

## License note

`convert_to_markdown` and `get_document_info` work without a license. In
evaluation mode the converted Markdown may be limited and carries an evaluation
notice — the conversion still succeeds. Add a license path to your config to
remove those limits:
...
| "No license configured" warnings | Expected in evaluation mode. Conversions still work; output may be limited and carries an evaluation notice. |
| `[Evaluation mode] Output may be limited …` prefix in answers | Running unlicensed. Set `GROUPDOCS_LICENSE_PATH` for unrestricted output. |
