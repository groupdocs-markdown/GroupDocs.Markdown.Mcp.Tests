...
> native `dnx` / global-tool install needs anything `apt-get`'d or `brew`'d on
> any platform.
...
docker pull ghcr.io/groupdocs-markdown/markdown-net-mcp:latest
...
```bash
docker inspect ghcr.io/groupdocs-markdown/markdown-net-mcp:latest \
...
`convert_to_markdown` and `get_document_info` with
...
may be limited and carries an evaluation notice
...
# - User: the non-root $APP_UID user from the .NET base image
```

The image runs as the non-root `$APP_UID` user provided by the
`mcr.microsoft.com/dotnet/aspnet:10.0` base image. If your mount's host
...
| `manifest unknown` / can't pull image | Version tag doesn't exist on that registry | Check [ghcr.io/groupdocs-markdown/markdown-net-mcp](https://github.com/groupdocs-markdown/GroupDocs.Markdown.Mcp/pkgs/container/markdown-net-mcp) for available tags. |
| `:latest` ran but didn't pick up a new release | Docker reuses cached image — `docker run :latest` does not auto-pull | `docker pull ghcr.io/groupdocs-markdown/markdown-net-mcp:latest` before running, or add `--pull always` to your `docker run`. |
