...
# Run all 20 tests locally against the LATEST nuget.org release (FASTEST + default)
./02_test-all-scenarios.sh

# Run tests in Docker containers (also defaults to latest)
./03_test-docker-compose.sh

# Try the published Docker MCP image with this repo's Files mounted (interactive)
...
# Run only ConvertToMarkdown tests
...
# Use a GroupDocs license (removes evaluation limits from converted Markdown)
