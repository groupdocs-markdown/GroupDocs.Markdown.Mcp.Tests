#!/bin/bash
################################################################################
# GroupDocs.Markdown.Mcp — Integration Test Runner (local)
#
...
#
# Scenarios:
#   - ToolDiscovery (server handshake, tool listing — exactly 2 tools)
#   - ConvertToMarkdown (DOCX/PDF/EPUB/XLSX conversion, image modes, passwords)
#   - GetDocumentInfo (JSON document info — format, page count, encryption)
...
#   --filter PATTERN        Run only tests matching pattern (e.g., ConvertToMarkdown)
...
#   ./02_test-all-scenarios.sh --version 26.5.0 --filter ConvertToMarkdown
...
TEST_PROJECT_DIR="$PROJECT_ROOT/src/GroupDocs.Markdown.Mcp.Tests"
...
main() {
    ...
    echo ""
    echo "╔════════════════════════════════════════════════════════════╗"
    echo "║  GroupDocs.Markdown.Mcp — Integration Test Runner (local) ║"
    echo "╚════════════════════════════════════════════════════════════╝"
    echo ""
