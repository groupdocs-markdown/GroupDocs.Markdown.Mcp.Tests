...
#
# Launches `ghcr.io/groupdocs-markdown/markdown-net-mcp` as an interactive MCP
...
#                         stdin and verify the server advertises its 3 tools
#                         (convert_to_markdown,
#                         get_document_info). Returns 0 on success.
#
# Prerequisites:
...
#                           advertise `convert_to_markdown` and `get_document_info`.
#   --license PATH          Mount a GroupDocs license file (removes evaluation
#                           limits from converted Markdown; without it the
#                           server runs in evaluation mode)
...
IMAGE_REPO="ghcr.io/groupdocs-markdown/markdown-net-mcp"
...
# storage path picks them up automatically. Mounted read-write — convert_to_markdown
# saves the generated .md (and any extracted images) back into /data.
docker_args+=(-v "$SAMPLE_DOCS_DIR:/data")
...
if [ -n "$LICENSE_PATH" ]; then
    ...
    log_info "License mounted — converted Markdown will be unrestricted"
else
    log_info "No license — converted Markdown may be limited and carries an evaluation notice"
fi
...
if [ "$RUN_MODE" = "smoke" ]; then
    ...
    if echo "$response" | grep -q '"convert_to_markdown"' && echo "$response" | grep -q '"get_document_info"'; then
        log_success "Expected tools advertised — smoke test PASSED"
        exit 0
    else
