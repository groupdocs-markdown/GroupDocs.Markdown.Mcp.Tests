...
public class ErrorHandlingTests : IClassFixture<McpServerFixture>
{
    ...
    [Fact]
    public async Task ConvertToMarkdown_UnknownFile_ReturnsErrorListingAvailableFiles()
    {
        ...
        var isErrorReported = (response.IsError ?? false)
            ...
            || body.Contains(SampleDocuments.BusinessPlanPdf, StringComparison.OrdinalIgnoreCase);
    ...
    [Fact]
    public async Task ConvertToMarkdown_CorruptedFile_DoesNotCrashServer()
    {
        ...
        // Server should either return a tool error string or throw an MCP error —
        // either way it must stay up and continue answering subsequent requests.
        try
        {
            var response = await _fixture.Client.CallToolAsync(
                catalog.ConvertToMarkdown.Name,
